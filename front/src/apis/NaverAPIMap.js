...
function NaverAPIMap({ detailTogOpen, detailTogClose, searchMarkerTogOpen }) {
    ...
    <a href="#" class="btn btn-outline-secondary navermap__btnSetCountOpt dropdown" role='button'>
        ...
        <div class="dropdown-menu navermap--countSettingDropdown">
            ...
            <p type='button' onclick="test.changeCountValue(2);">5000개</p> <p type='button' onclick="test.changeCountValue(1);">3000개</p> <p type='button' onclick="test.changeCountValue(0);">1000개</p>
            </div>
        </div>
    </a>`;
    ...
    // 마커리스트 띄우기
    const setMarkerList = (markerList)=>{
        for (
            let idx = 0;
            idx < markerList.length;
            idx++
        ) {
            markerList[idx] = new naver.maps.Marker({
                ...
                icon: {
                    content: [
                        ...
                        "<span>",
                        checkStringCount(markerList[idx].name, 5),
                        "</span>",
                        "</div>",
                        '<div class="naverApiMap-mappingMarker--phoneNumberZone">',
                        "<span>",
                        markerList[idx].tel.substring(0, 2) === "02" ?
                            markerList[idx].tel.replace(
                                /(\d{2})(\d{3,4})(\d{4})/,
                                "$1-$2-$3"
                            )
                            : markerList[idx].tel.replace(
                ...
                title: markerList[idx].businessNumber
            });

            //마커 클릭 이벤트
            naver.maps.Event.addListener(markerList[idx], "click", function (e) {
                searchMarkerTogOpen(searchList);
                detailTogClose();
                detailTogOpen(e.overlay.title);
            })
            // test.resultMarkers.push(markerList[idx]);
        }
    }
    ...
    const setPlaceMarker = (placeinfo, stateTodata) => {
        ...
        placeMarker = new naver.maps.Marker({
            ...
            // icon:{url:'./img/pin_default.png',size:new naver.maps.Size(22,35),orgin:new naver.maps.Point(0,0), anchor:new naver.maps.Point(11,35)},
                ...
                if (state == null) {
                    ...
                    instance({
                        ...
                        for (let idx = 0; idx < res.data.length; idx++) {
                            markerList.push(res.data[idx]);
                            searchList.push(res.data[idx]);
                        }

                        //마커 출력
                        setMarkerList(markerList);
                        //왼쪽에 searchResultList 뜨도록
                        searchMarkerTogOpen(searchList);

                        test.markers.push(markerList);
                        test.searchData.push(searchList);
                        test.doCluster();

                        document.querySelector('#btnSearchListOpen').addEventListener('click', function () { searchMarkerTogOpen(searchList) });
                    }).catch(() => {
                        searchList = [];
                    ...
                    naver.maps.Event.addDOMListener(
                        ...
                        function (e) {
                            ...
                            instance({
                                ...
                                //마커 리스트 왼쪽에 searchResultList로 뜨도록
                                searchMarkerTogOpen(searchList);
